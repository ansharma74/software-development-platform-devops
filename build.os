...
#################### Script Header #########################
#Script Name    : build.os
#Description    : usage: build.os wr | nos | linux | lxde target
...
then
    ...
    echo   target choice: "x86_64 | i686 | arm | mips"
    echo i.e.:
    echo   build.os nos x86_64
    echo
    exit 1
fi
...
mkdir -p $TOP/buildoutput/$OS
rm -rf $TOP/images/$OS
mkdir -p $TOP/images/$OS
mkdir -p $TOP/images/$OS/etc
cp $TOP/boot/etc/ld.so.conf $TOP/images/$OS/etc/ld.so.conf
mkdir -p $TOP/images/$OS/etc/ld.so.conf.d/
...
echo  " ---------- Setting up Env --------------"
export PATH=$PREFIX/bin:$PREFIX/usr/bin:$PREFIX/sbin:$PREFIX/usr/sbin:$TCHAIN/usr/sbin:$TCHAIN/usr/bin:$TCHAIN/sbin:$TCHAIN/bin:$TCHAIN/$TARGET/bin:$TCHAIN/libexec/gcc/$TARGET/4.8.1
export PKG_CONFIG_PATH=$TCHAIN/lib/pkgconfig:$TCHAIN/lib64/pkgconfig:$PREFIX/lib/pkgconfig:$PREFIX/lib64/pkgconfig:$PREFIX/usr/lib/pkgconfig:$PREFIX/usr/lib64/pkgconfig
export LD_LIBRARY_PATH=$TCHAIN/lib:$TCHAIN/lib64:$TCHAIN/usr/lib:$TCHAIN/usr/lib64:$TCHAIN/libexec:$PREFIX/lib:$PREFIX/lib64:$PREFIX/usr/lib:$PREFIX/libexec:$PREFIX/usr/lib64:$TCHAIN/$HOST/$TARGET/lib:$TCHAIN/libexec/gcc/$TARGET/4.8.1:$TCHAIN/lib/gcc/$TARGET/4.8.1
export C_INCLUDE_PATH=$TCHAIN/usr/include:$TCHAIN/include:$PREFIX/include:$PREFIX/usr/include:$TCHAIN/lib/gcc/$TARGET/4.8.1/include
export LIBRARY_PATH=$TCHAIN/lib:$TCHAIN/lib64:$TCHAIN/usr/lib:$TCHAIN/usr/lib64:$TCHAIN/libexec:$PREFIX/lib:$PREFIX/lib64:$PREFIX/usr/lib:$PREFIX/libexec:$PREFIX/usr/lib64:$TCHAIN/$HOST/$TARGET/lib:$TCHAIN/libexec/gcc/$TARGET/4.8.1:$TCHAIN/lib/gcc/$TARGET/4.8.1
export CXX_INCLUDE_PATH=$PREFIX/usr/include:$PREFIX/include:$TCHAIN/usr/include:$TCHAIN/include:$TCHAIN/lib/gcc/$TARGET/4.8.1/include
export CFLAGS="-fPIC -I$PREFIX/usr/include -I$PREFIX/include -I$TCHAIN/usr/include -I$TCHAIN/include -I$TCHAIN/lib/gcc/$TARGET/4.8.1/include"
export CXXFLAGS="-fPIC -I$PREFIX/usr/include -I$PREFIX/include -I$TCHAIN/usr/include -I$TCHAIN/include -I$TCHAIN/lib/gcc/$TARGET/4.8.1/include"
export LDFLAGS="-L$PREFIX/lib -L$PREFIX/lib64 -L$PREFIX/usr/lib -L$PREFIX/libexec -L$PREFIX/usr/lib64 -L$TCHAIN/lib -L$TCHAIN/lib64 -L$TCHAIN/usr/lib -L$TCHAIN/$HOST/$TARGET/lib -L$TCHAIN/usr/lib64 -B$TCHAIN/lib/gcc/$TARGET/4.8.1 -B$TCHAIN/libexec/gcc/$TARGET/4.8.1" 
}
...
function build_nos_linux_component {
    ...
    acl)
        ...
        ./configure --prefix=/usr --with-pic --enable-lib64=yes \
          --enable-static=no --enable-gettext=no CPPFLAGS="-I$PREFIX/include" \
          LDFLAGS="-L$PREFIX/libexec64 -L$PREFIX/lib64"
        make
    ...
    libdlo)
        rm -rf $TOP/buildoutput/$DIR/libdlo
        mkdir $TOP/buildoutput/$DIR/libdlo
        cd $TOP/buildoutput/$DIR/libdlo
...
if [ ! -d $TOP/images/builds ]
then
    mkdir $TOP/images/builds
fi
mv $LINUX_INSTALL_DIR/cbirwr.iso $PREFIX/builds/cbirwr-$TARGET_ARCH.iso
fi
...
mv $LINUX_INSTALL_DIR/cbirnos.iso $PREFIX/builds/cbirnos-$TARGET_ARCH.iso
fi
...
mv $LINUX_INSTALL_DIR/cbirlxde.iso $PREFIX/builds/cbirlxde-$TARGET_ARCH.iso
fi
...
for pack in $tpd_X_packages; do
    buildTPD_display_component $pack linux
done        
...
mv $LINUX_INSTALL_DIR/cbirlinux.iso $PREFIX/builds/cbirlinux-$TARGET_ARCH.iso
fi
